...
from database import fetch_sensor_data, fetch_sensor_data_last_minute, fetch_sensor_data_range
import threading
import time
...
@socketio.on('request_chart_data_range')
def send_chart_data_range(data):
    sensor_type = data.get('sensor_type')
    start_time = data.get('start_time')
    end_time = data.get('end_time')
    print(f"Получен запрос на данные для датчика {sensor_type} с {start_time} по {end_time}")
    fetched_data = fetch_sensor_data_range(sensor_type, start_time, end_time)
    socketio.emit('chart_data_range', {'sensor_type': sensor_type, 'data': fetched_data})
